...
async def admin_callback(callback: types.CallbackQuery):
    ...
    if action == "users":
        ...
        payments = await get_all_payments(limit=50)
        ...
        total_accounts = 0
        for u in users:
            total_accounts += len(await get_user_accounts(u.tg_user_id))
...
async def give_subscription_user_id(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("Введите количество дней подписки (целое число):")
...
async def give_subscription_days(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"✅ Пользователю {target_user_id} выдана подписка на {days} дней (до {new_end.strftime('%d.%m.%Y')})")
    except Exception as e:
        await message.answer(f"❌ Ошибка: {e}")
    finally:
        await state.finish()
...
async def create_promo_code(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введите количество дней подписки, которое даёт промокод:")
...
async def create_promo_days(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("Введите максимальное количество активаций (1 - для одного пользователя):")
...
async def create_promo_max_uses(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("Введите срок действия в формате ДД.ММ.ГГГГ или '-' для бессрочного:")
...
def register_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(give_subscription_start, text="admin_give_sub")
